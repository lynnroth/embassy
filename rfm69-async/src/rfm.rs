    ...
    DELAY: DelayNs,
{
    ...
    /// Set the TX output power by writing the `RegPaLevel` (0x11) register.
    ///
    /// The caller provides the raw register value. For the RFM69HCW:
    /// - `0x5F` = PA1 on, output_power=31 = +13 dBm (CircuitPython default)
    /// - `0x7F` = PA1+PA2 on, output_power=31 = +17 dBm
    /// - `0x7F` + boost registers = +20 dBm (use `set_tx_power_boost` instead)
    pub async fn set_tx_power(&mut self, pa_level: u8) -> Result<(), Error<E, RESET::Error, DIO0::Error>> {
        self.write_register(Register::PaLevel, pa_level).await
    }

    /// Set TX power to maximum (+20 dBm) on the RFM69HCW by enabling PA1+PA2
    /// with the high-power boost registers.
    ///
    /// This writes:
    /// - PaLevel (0x11) = 0x7F (PA1+PA2, max output power)
    /// - Ocp (0x13) = 0x0F (overcurrent protection for high power)
    /// - TestPa1 (0x5A) = 0x5D (PA1 boost)
    /// - TestPa2 (0x5C) = 0x7C (PA2 boost)
    pub async fn set_tx_power_boost(&mut self) -> Result<(), Error<E, RESET::Error, DIO0::Error>> {
        self.write_register(Register::PaLevel, 0x7F).await?;
        self.write_register(Register::Ocp, 0x0F).await?;
